...
namespace GameClient
{
    ...
    internal class ActivityJarFrame : ClientFrame
    {
                ...
                if (this.m_currActJar != null)
                {
                    ...
                    if (var.m_index >= 0 && var.m_index < arrBonusItems.Count)
                    {
                        ...
                        if (itemData != null)
                        {
                            ...
                            {
                                DataManager<ItemTipManager>.GetInstance().ShowTip(var2, null, TextAnchor.MiddleCenter, true, false, true);
                                if (this.m_currActJar.jarData.eType == ProtoTable.JarBonus.eType.EquipJar)
                                {
                                ...
                                else if (this.m_currActJar.jarData.eType == ProtoTable.JarBonus.eType.FashionJar)
                                {
        ...
        private void _InitMainTabs()
        {
            ...
            {
                this.m_objOverlay0.SetActive(!this.m_objOverlay0.activeSelf);
            });
            this.m_objOverlay0.SetActive(false);

            ComUIListScript comList = this.m_objOverlay0.GetComponent<ComUIListScript>();
            comList.Initialize();
            comList.onItemChageDisplay = delegate (ComUIListElementScript var, bool selected)
            {
                ComCommonBind component = var.GetComponent<ComCommonBind>();
                StaticUtility.SafeSetVisible(component, "selected", selected);
            };
            comList.onItemVisiable = delegate (ComUIListElementScript var)
            {
                if (var.m_index >= 0 && var.m_index < this.m_arrEquipActData.Count)
                {
                    ActivityJarFrame.ActJarInfo actJarInfo = this.m_arrEquipActData[var.m_index];
                    ...
                    {
                        comList.SelectElement(var.m_index, true);
                        _UpdateEquipmentJarInfo(m_arrEquipActData[var.m_index], false);
            ...
            this.m_togTab0.onValueChanged.AddListener(delegate(bool var)
            {
                if (var)
                {
                    this._UpdateEquipmentJarInfo(this.m_selectEquipActJar, true);
                    if (this.m_contentDescText)
                    {
                        this.m_contentDescText.text = TR.Value("jar_content_desc_pink");
                    }
                }
                this.m_objOverlay0.SetActive(false);
            });
            ...
            {
                this.m_objOverlay1.SetActive(!this.m_objOverlay1.activeSelf);
            });
            this.m_objOverlay1.SetActive(false);
            ComUIListScript comList1 = this.m_objOverlay1.GetComponent<ComUIListScript>();
            comList1.onItemChageDisplay = delegate(ComUIListElementScript var, bool selected)
            {
                ComCommonBind component = var.GetComponent<ComCommonBind>();
                StaticUtility.SafeSetVisible(component, "selected", selected);
            };
            comList1.onItemVisiable = delegate (ComUIListElementScript var)
            {
                if (var.m_index >= 0 && var.m_index < this.m_arrFashionActData.Count)
                {
                    ActivityJarFrame.ActJarInfo actJarInfo = this.m_arrFashionActData[var.m_index];
                    Utility.GetComponetInChild<Text>(var.gameObject, "Label").text = actJarInfo.jarData.strName;
                    Button component = var.GetComponent<Button>();
                    component.onClick.RemoveAllListeners();
                    component.onClick.AddListener(delegate ()
                    {
                        comList1.SelectElement(var.m_index, true);
            ...
            this.m_togTab1.onValueChanged.AddListener(delegate (bool var)
            {
                if (var)
                {
                    ...
                    if (this.m_contentDescText)
                    {
                        this.m_contentDescText.text = TR.Value("jar_content_desc_pupple");
                    }
        ...
        private void _UpdateTabs(bool bIsInit = false)
        {
            ...
            while (enumerator.MoveNext())
            {
                ...
                if (activityInfo != null && activityInfo.state != 0)
                {
                    ...
                    if (jarData.eType == ProtoTable.JarBonus.eType.EquipJar)
                    {
                    ...
                    else if (jarData.eType == ProtoTable.JarBonus.eType.FashionJar)
                    {
            ...
            for (int i = 0; i < this.m_currActJar.jarData.arrBuyInfos.Count; i++)
            {
                ...
                component.onClick.AddListener(delegate ()
                {
                    ...
                    UnityAction unityAction = delegate ()
                    {
                        ...
                        {
                            DataManager<JarDataManager>.GetInstance().RequestBuyJar(m_currActJar.jarData, buyInfo, ActivityJarFrame.GetArtifactActivityID(), 0U);
